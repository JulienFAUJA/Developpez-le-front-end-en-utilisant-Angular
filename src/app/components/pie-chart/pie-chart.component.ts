...
export class PieChartComponent implements OnInit, OnChanges, OnDestroy {
  ...
  createChart() {
    ...
    function GetEndPoint(
      ...
      let lengthX =
        quadrant === 1 || quadrant === 4 ? startLenght : -startLenght;
      let lengthY =
        quadrant === 3 || quadrant === 2 ? startLenght : startLenght;
    ...
    if (orientation.type === 'portrait-primary') {
      ...
      canvas?.setAttribute(
        'height',
        (window.screen.availHeight * 0.8).toString()
      );
      label_enabled = true;
      ...
      afterDraw(chart: Chart, args: {}, options: {}) {
        ...
        if (label_enabled === false) {
          ...
          const position: TextPosition = {
            ...
            name: texte,
            id: index // Vous pouvez attribuer un identifiant unique à chaque texte
          };
    ...
    ctx.canvas.addEventListener('click', (event) => {
      ...
      for (const position of textPositions) {
        if (clickX > position.x && clickX < (position.x + position.width) && clickY > (position.y-position.height) && clickY < (position.y + position.height)) {
          console.log('Clic sur le texte:', position.name);
          console.log('ID du texte:', position.id);
            ...
            listeners: {
              click: function (context, event) {
                ...
                const page_id: number =
                  self.countryData[context.dataIndex].id - 1;
                console.log(
                  'country:',
                  self.countryData[context.dataIndex].country,
                  'id:',
                  page_id,
                  'index:',
                  context.dataIndex
